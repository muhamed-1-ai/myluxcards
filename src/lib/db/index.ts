...
export async function withTransaction<T>(work: (client: PoolClient) => Promise<T>, isolation: "read committed"|"repeatable read"|"serializable" = "read committed") {
...
export async function databaseHealth() { return (await query<{ ok:boolean }>("select true as ok")).rows[0]?.ok === true; }
export async function closeDatabasePool() { await pool.end(); if (globalPool.__myluxcardsPool===pool) delete globalPool.__myluxcardsPool; }
