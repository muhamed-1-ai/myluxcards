      ...
      <main className="dash-main">
        ...
        {tab === "cards" && <section>
          ...
          <div className="table-card">
            ...
            {visible.some(card => !card.activatedAt && /^[0-9a-f-]{36}$/i.test(card.id)) && <div className="activation-box"><div><strong>Activate a delivered card</strong><span>Enter the one-time code supplied by MyLuxCards. We’ll match it to the correct card automatically.</span></div><input value={activationCode} onChange={event=>setActivationCode(event.target.value.toUpperCase().replace(/\s/g,""))} placeholder="MLC-12AB34CD" autoComplete="off"/><button disabled={!/^MLC-[0-9A-F]{8}$/i.test(activationCode)} onClick={async()=>{const response=await fetchWithSessionRefresh("/api/cards/activate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:activationCode})});const payload=await response.json().catch(()=>({}));if(!response.ok){notify(payload.message||"Activation failed.");return;}setCards(cards.map(x=>x.id===payload.cardId?{...x,active:true,activatedAt:new Date().toISOString(),expiry:payload.expiry.slice(0,10)}:x));setActivationCode("");notify(`/${payload.slug} is activated and ready to share.`);}}>Activate</button></div>}
