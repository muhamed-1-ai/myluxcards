...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (typeof body.id !== "string" || typeof body.disabled !== "boolean") return Response.json({ message: "Invalid request." }, { status: 400 });
    const before = await supabaseJson(`/rest/v1/profiles?id=eq.${body.id}&role=eq.CUSTOMER&select=id,disabled&limit=1`, {}, true);
