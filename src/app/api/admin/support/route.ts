...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (reply) {
      ...
      } else return Response.json({ message:"Email delivery is not configured. Add RESEND_API_KEY and EMAIL_FROM before replying." }, { status:503 });
      ...
      changes.last_reply_at = emailedAt; changes.status = body.status && statuses.has(body.status) ? body.status : "WAITING_CUSTOMER";
    }
