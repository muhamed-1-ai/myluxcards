...
export async function POST(request:Request){
  ...
  try{
    const body=await request.json().catch(()=>({}));const company=cleanText(body.companyName,180),contact=cleanText(body.contactPerson,140),email=cleanText(body.businessEmail,320),phone=cleanText(body.phone,30),source=cleanText(body.leadSource,100),notes=cleanText(body.notes,2000);const quantity=Number(body.estimatedQuantity);
    ...
    const normalizedCompany=company.toLowerCase().replace(/[^a-z0-9]/g,"");const emailHash=hash(normalizeEmail(email)),phoneHash=phone?hash(phone.replace(/\D/g,"")):null;
