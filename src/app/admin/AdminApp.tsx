...
export default function AdminApp({ identity }:{identity:AdminIdentity}) {
  ...
  return <div className="admin-shell">
    ...
    <aside className={mobile?"open":""}><p>CONTROL CENTRE</p><nav>{allowed.map(item=><button key={item} className={section===item?"active":""} onClick={()=>navigate(item)}>{labels[item]}</button>)}<a href="/admin/physical-cards">QR &amp; NFC Management</a><a href="/admin/affiliates">Affiliate program</a></nav><button className="admin-logout" onClick={logout}>Log out</button></aside>
