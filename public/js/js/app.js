...
class LuxApp {
  ...
  initConfigurator() {
    ...
    document.getElementById('save-card-design')?.addEventListener('click', () => this.showToast('Your card design is saved in this browser.', 'success'));
  }
      ...
      return `
        <div class="drawer-item glass">
          ...
          <div class="drawer-item-actions">
            <div class="quantity-control" aria-label="Quantity for ${card.title}">
            ...
            <button class="drawer-item-remove" type="button" data-cart-id="${card.id}" aria-label="Remove ${card.title}"><i data-lucide="trash-2"></i></button>
          </div>
        </div>
      `;
    }).join('');
